...
{"latitude": "52.833502", "user_id": 25, "name": "David Behan", "longitude": "-8.522366"}
  ...
  filter_customers(){
    this.customers.forEach(element => {
     this.distt=this.distance(element.latitude,element.longitude);
     var res = this.distt.split("-");
            if(res[1]=='km'){
              if(res[0]<=100)
              {
                element.distance=this.distt;
                this.finalcustomers.push(element);
              }
            }
            if(res[1]=='m'){
              if(res[0]<=100000)
        ...
        console.log(this.finalcustomers);
      }
